...
function nuevoUsuario(nombre, email, password) {
  ...
  // Devuelve el objeto
  // Tu código:
  const Nusuar={nombre,email,password};
  return Nusuar;
...
function pasarUsuarioAPremium(usuarios) {
  ...
  // Tu código:
  for(let cont=0;cont<usuarios.length;cont++){usuarios[cont].esPremium=true}
  return usuarios;
}
...
function sumarLikesDeUsuario(usuario) {
  ...
  // Tu código:
  let sumas=0
  for(let i=0;i<usuario.posts.length;i++){sumas+=usuario.posts[i].likes;}
  return sumas;
}
...
function agregarMetodoCalculoDescuento(producto) {
  ...
  // Tu código:
  producto.calcularPrecioDescuento= function(){let ecual=this.precio-(this.precio*this.porcentajeDeDescuento); return ecual;}
 return producto;
}
